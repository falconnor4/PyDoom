...
WIDTH = 800
HEIGHT = 600 #I divided these by 2 just to make rendering faster while demo-ing
...
@dataclass(frozen=True) #makes vertices immutable for rendering
class Vertex:
    x: float
    y: float
...
#Initialize a Thing object with specified coordinates, angle, and type.

#Parameters:
#    x (float): The x-coordinate of the Thing.
#    y (float): The y-coordinate of the Thing.
#    angle (float): The angle in degrees, converted to radians.
#    type_ (str): The type identifier for the Thing.
...
class Map:
    ...
    def extract_entries(self, filepath, mapname):
        ...
        self.entry_data = {}
        inmap = False
        bmapname = bytes([ord(c) for c in mapname])
